...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install. Quarantine machinery has been removed —
    the hermes-updater now owns Windows .exe safety."""
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if method == "docker":
        print("Docker installs are updated via 'docker pull'. See docs.")
        sys.exit(1)
    elif method in ("nixos", "homebrew", "pip"):
        print(f"Use your package manager to update ({method}).")
        sys.exit(1)

    # Git checkout: check for auto-adopt, then dev_update, then hermes-updater
    ...
    if not _in_place and git_dir.exists():
        ...
        if should_use_worktree_update(PROJECT_ROOT, in_place=False):
            ...
            if _result.fell_back:
                # Worktree creation failed — fall through to hermes-updater.
                print("→ Continuing with hermes-updater...")
            else:
                # Cancelled or merge conflict — stop here.
                return

    # Route to hermes-updater for managed slots and git checkouts.
    # The updater owns: backup, git pull, dependency sync, skills sync,
    # gateway restart, and all the safety machinery (quarantine, hangup
    # protection, concurrent detection, etc.) that was previously inline.
    from hermes_cli.updater_compat import run_updater

    run_updater(args, gateway_mode=gateway_mode)
